...
from teragrid.glue2.pbs import *
from teragrid.glue2.moab import *
...
activities = MoabJobsAgent().run()

private_doc = TeraGridGlue2Agent().run(EntitiesAgent().run(activities))

shares = PbsQueuesAgent().run(activities)
endpoints = []  # currently, no GRAM endpoints at NICS
service = PbsComputingServiceAgent().run(shares + endpoints)
exec_envs = PbsExecutionEnvironmentsAgent().run()
manager = PbsComputingManagerAgent().run(shares + exec_envs + service)
