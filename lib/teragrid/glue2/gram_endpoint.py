...
logger = logging.getLogger("GramEndpointsAgent")

##############################################################################################################

class GramEndpointsAgent(ComputingEndpointsAgent):
    def __init__(self, args={}):
        ComputingEndpointsAgent.__init__(self,args)
        self.name = "teragrid.glue2.GramEndpointsAgent"
    ...
    def run(self, docs_in=[]):
        ...
        try:
            ...
            logger.error("failed to read registered kits file "+kits_file)
            raise AgentError("failed to read registered kits file "+kits_file)
        ...
        for kit_dir in kit_dirs:
            if kit_dir.find("ctss-remote-compute") >= 0:
                ...
                break
        if service_dir == None:
            logger.info("CTSS remote compute kit not found in registered kits file "+kitsFile)
            return
            ...
            if entry.endswith(".conf"):
                if entry.find("gram") != -1:
                    ...
                    try:
                        ...
                        logger.warn("failed to read kit conf file "+file_name)
                        continue
                    endpoints.append(self._createEndpoint(reg_info,support_level))

        for endpoint in endpoints:
            endpoint.id = self._getSystemName()
    ...
    def _createEndpoint(self, reg_info, quality_level):
        ...
        try:
            ca_dir = self.config.get("globus","ca_certificates_directory")
        except ConfigParser.Error:
            pass
        ...
        try:
            host_cert_file = self.config.get("globus","host_certificate")
        except ConfigParser.Error:
            pass
        container_cert_file = "/etc/grid-security/containercert.pem"
        try:
            container_cert_file = self.config.get("globus","container_certificate")
        except ConfigParser.Error:
            pass
    ...
    def _getIssuer(self, caFile):
        ...
        try:
            grid_cert_info = self.config.get("globus","grid-cert-info")
        except ConfigParser.Error:
            pass
...
if __name__ == "__main__":
    agent = GramEndpointsAgent.createFromCommandLine()
