...
from teragrid.glue2.loadleveler import *
...
shares = LoadLevelerQueuesAgent().run(activities)
endpoints = GramEndpointsAgent().run()
service = LoadLevelerComputingServiceAgent().run(shares + endpoints)
exec_envs = LoadLevelerExecutionEnvironmentsAgent().run()
manager = LoadLevelerComputingManagerAgent().run(shares + exec_envs + service)
